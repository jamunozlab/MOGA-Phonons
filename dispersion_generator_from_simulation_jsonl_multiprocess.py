#!/usr/bin/env python3
"""
Generate phonon dispersions from MOGA JSONL generation history.

This script can process either:

1. One simulation directory:

    python dispersion_generator_from_simulation_jsonl.py \
        --simulation simulations/simulation000378

2. All simulations listed in an experiment file:

    python dispersion_generator_from_simulation_jsonl.py \
        --experiment experiment000007

For each simulation, the script:

- reads a_val and atomic_masses from inputc
- reads generation_history.jsonl
- selects best/all/top-k GA candidates per generation
- builds the full BCC force-constant tensor in memory
- passes force constants directly to Phonopy using the Python API
- writes band_ga.yaml and dispersion.png outputs
- can process selected candidates in parallel with --nproc

Default output layout for --selection best:

    simulations/simulationXXXXXX/phonon_generations/generation_000001/band_ga.yaml
    simulations/simulationXXXXXX/phonon_generations/generation_000001/dispersion.png

Output layout for --selection top-k or --selection all:

    simulations/simulationXXXXXX/phonon_generations/generation_000001/solution_000032_rank_000/band_ga.yaml
    simulations/simulationXXXXXX/phonon_generations/generation_000001/solution_000032_rank_000/dispersion.png

The subdirectory name records the original population index and the rank among selected
solutions for that generation.
"""
...
def read_inputc(input_path):
    ...
    atomic_masses = [
        float(x) for x in re.split(r"[,\s]+", params["atomic_masses"])
        if x
    ]

    return a_val, np.array(atomic_masses, dtype=float)
...
def fitness_score(fitness, sort_by):
    """Return scalar score used to rank candidates within a generation."""
    fitness = np.array(fitness, dtype=float)

    if sort_by == "fitness_norm":
        return float(np.linalg.norm(fitness))
    if sort_by == "fitness1":
        return float(fitness[0])
    if sort_by == "fitness2":
        return float(fitness[1])
    if sort_by == "fitness3":
...
def _record_from_solution(data, solution, fitness, solution_index, rank=None, source="population", sort_by="fitness_norm"):
    """Build one normalized candidate record from a JSONL generation entry."""
    fc5 = np.array(solution, dtype=float)
    fitness = np.array(fitness, dtype=float)

    if fc5.size != 5:
        raise ValueError(
            f"Expected 5 force constants for generation {data.get('generation')}, "
            f"solution {solution_index}; got {fc5.size}: {fc5}"
        )
    ...
    return {
        ...
        "sort_by": sort_by,
        "sort_score": fitness_score(fitness, sort_by),
        "raw_generation_metadata": {
            key: value
            for key, value in data.items()
            if key not in {"population", "fitnesses"}
        },
    }


def read_generation_history_jsonl(
    generation_history_path,
    selection="best",
    top_k=5,
    sort_by="fitness_norm",
    every_n_generations=1,
    max_generations=None,
):
    """
    Read candidate force-constant vectors from generation_history.jsonl.

    Parameters
    ----------
    generation_history_path : Path
        Path to generation_history.jsonl.
    selection : {"best", "top-k", "all"}
        Candidate-selection mode.
    top_k : int
        Number of candidates per generation when selection="top-k".
    sort_by : {"fitness_norm", "fitness1", "fitness2", "fitness3"}
        Ranking score for selection="top-k".
    every_n_generations : int
        Process every Nth generation. Generation 1 is always eligible.
    max_generations : int or None
        Stop after this many generations have been seen in the JSONL file.

    Returns
    -------
    records : list[dict]
        Normalized candidate records. Each record contains generation,
        solution_index, rank, fc5, fitness, fitness_norm, and metadata.
    """
    records = []
    ...
    with open(generation_history_path, "r") as f:
        for line_number, line in enumerate(f, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"Could not parse JSON on line {line_number} of "
                    f"{generation_history_path}: {exc}"
                ) from exc
            ...
            if every_n_generations > 1 and (generation - 1) % every_n_generations != 0:
                continue

            if selection == "best":
                if "best_solution" not in data or "best_fitness" not in data:
                    raise KeyError(
                        f"Missing best_solution/best_fitness in line {line_number} "
                        f"of {generation_history_path}"
                    )
                ...
                records.append(
                    _record_from_solution(
                        ...
                        solution_index=int(data.get("best_solution_index", -1)),
                        rank=0,
            ...
            elif selection in {"top-k", "all"}:
                if "population" not in data or "fitnesses" not in data:
                    raise KeyError(
                        f"Missing population/fitnesses in line {line_number} "
                        f"of {generation_history_path}"
                    )
                ...
                if len(population) != len(fitnesses):
                    raise ValueError(
                        f"Generation {generation} has {len(population)} population "
                        f"entries but {len(fitnesses)} fitness entries."
                    )

                generation_records = []
                for solution_index, (solution, fitness) in enumerate(zip(population, fitnesses)):
                ...
                # Larger fitness values are better for all three current objectives,
                # and therefore also for the Euclidean norm.
                generation_records.sort(
                    key=lambda record: record["sort_score"],
                    reverse=True,
                )

                if selection == "top-k":
                    generation_records = generation_records[:top_k]

                for rank, record in enumerate(generation_records):
                    record["rank"] = rank
                ...
                raise ValueError(
                    f"Unknown selection='{selection}'. Use 'best', 'top-k', or 'all'."
                )

    if not records:
        raise ValueError(f"No records found in {generation_history_path}")

    return records


# -----------------------------------------------------------------------------
# BCC force constants and Phonopy helpers
# -----------------------------------------------------------------------------
...
def write_band_yaml(
    ...
    """Write a compact band_ga.yaml file with candidate metadata."""
    fc5 = record["fc5"]

    data = {
        "generation": int(record["generation"]),
        "solution_index": int(record["solution_index"]),
        "rank": None if record["rank"] is None else int(record["rank"]),
        ...
        "selection_metadata": {
            ...
            "sort_score": float(record["sort_score"]),
            "fitness": [float(x) for x in record["fitness"]],
            "fitness_norm": float(record["fitness_norm"]),
        },
        "force_constants_5": {
            "alpha0": float(fc5[0]),
            "alpha1": float(fc5[1]),
        ...
        "raw_generation_metadata": record.get("raw_generation_metadata", {}),
        "phonon": [],
    }
...
def plot_dispersion(
    ...
    text = (
        ...
        f"idx = {record['solution_index']}\n"
        f"rank = {record['rank']}\n"
        f"|f| = {record['fitness_norm']:.3g}\n"
        f"m = {mass:g}\n"
        f"a = {a_val:g}"
    )
...
def output_directory_for_record(output_root, record, selection):
    """Return output directory for one selected record."""
    generation_dir = output_root / f"generation_{record['generation']:06d}"

    if selection == "best":
        return generation_dir

    rank = 0 if record["rank"] is None else int(record["rank"])
    solution_index = int(record["solution_index"])

    return generation_dir / f"solution_{solution_index:06d}_rank_{rank:03d}"
...
def process_candidate_task(task):
    """
    Process one selected GA candidate.

    This function is intentionally top-level so it can be used safely with
    multiprocessing on macOS, Linux, and clusters that use spawn semantics.
    """
    record = task["record"]
    output_root = Path(task["output_root"])
    selection = task["selection"]
    a_val = float(task["a_val"])
    mass = float(task["mass"])
    system_size = int(task["system_size"])
    band_points = int(task["band_points"])
    write_fc = bool(task["write_force_constants"])
    skip_existing = bool(task["skip_existing"])

    candidate_dir = output_directory_for_record(output_root, record, selection)
    band_yaml_path = candidate_dir / "band_ga.yaml"
    dispersion_path = candidate_dir / "dispersion.png"

    if skip_existing and band_yaml_path.is_file() and dispersion_path.is_file():
        return {
            "status": "skipped",
            "generation": int(record["generation"]),
            "solution_index": int(record["solution_index"]),
    ...
    if write_fc:
        force_constants_path = candidate_dir / "FORCE_CONSTANTS"
...
# -----------------------------------------------------------------------------
# Simulation/experiment orchestration
# -----------------------------------------------------------------------------
...
def process_simulation(simulation_path, args):
    ...
    generation_history_path = simulation_path / args.generation_history_filename
    ...
    if not generation_history_path.is_file():
        print(
            f"WARNING: Skipping {simulation_path.name}; "
            f"missing {generation_history_path.name}"
        )
        return 0
    ...
    records = read_generation_history_jsonl(
        generation_history_path,
        selection=args.selection,
        top_k=args.top_k,
        sort_by=args.sort_by,
        every_n_generations=args.every_n_generations,
    ...
    if args.selection == "top-k":
        print(f" top_k = {args.top_k}, sort_by = {args.sort_by}")
    ...
    for record in records:
        if args.verbose:
            print(
                f" Queued generation {record['generation']}, "
                f"solution {record['solution_index']}, "
                f"rank {record['rank']}: {record['fc5']}"
            )
        ...
        tasks.append(
            {
                ...
                "write_force_constants": args.write_force_constants,
    ...
    if args.nproc == 1:
        for task in tasks:
            result = process_candidate_task(task)
        ...
        with ProcessPoolExecutor(max_workers=args.nproc) as executor:
            futures = [executor.submit(process_candidate_task, task) for task in tasks]

            for i, future in enumerate(as_completed(futures), start=1):
                try:
                    result = future.result()
                except Exception as exc:
                    print(f"ERROR: candidate task failed in {simulation_path.name}: {exc}")
                    raise
                ...
                if args.progress and (i % args.progress_every == 0 or i == len(futures)):
                    print(
                        f" {simulation_path.name}: completed {i}/{len(futures)} "
                        f"tasks; written={n_written}, skipped={n_skipped}"
                    )

    print(f" Wrote {n_written} candidate directories; skipped {n_skipped} existing.")
    return n_written
...
def resolve_repo_root(repo_root_arg):
    ...
    Default is the directory containing this script, which works if the script
    lives at the top level of MOGA-Phonons.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Generate phonon dispersions from MOGA JSONL generation histories. "
    ...
    mode.add_argument(
        ...
        help=(
            ...
            "simulations/simulation000378 or simulation000378."
        ),
    )
    ...
    parser.add_argument(
        "--generation_history_filename",
        type=str,
        default="generation_history.jsonl",
        help="Name of JSONL MOGA generation-history file inside each simulation directory.",
    )
    ...
    parser.add_argument(
        ...
        help="Directory created inside each simulation path for per-candidate outputs.",
    )
    ...
    parser.add_argument(
        ...
        default="best",
        help=(
            "Which GA candidates to process. "
            "'best' writes one dispersion per generation. "
            "'top-k' writes the top K candidates per generation. "
            "'all' writes every population member per generation."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of candidates per generation when --selection top-k.",
    )

    parser.add_argument(
        "--sort-by",
        choices=["fitness_norm", "fitness1", "fitness2", "fitness3"],
        default="fitness_norm",
        help="Ranking score for --selection top-k. Larger is treated as better.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Only process every Nth generation. Generation 1 is included.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only process generations up to this generation number.",
    )
    ...
    parser.add_argument(
        "--show",
        action="store_true",
        help="Show each plot interactively. Usually not recommended for many candidates.",
    )

    parser.add_argument(
        "--write_force_constants",
        action="store_true",
        help="Also write FORCE_CONSTANTS to each candidate directory.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Number of worker processes for dispersion generation. "
            "Use 1 for serial execution. Default: max(1, os.cpu_count()-2)."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip a candidate if both band_ga.yaml and dispersion.png already exist. "
            "Useful for restarting interrupted runs."
        ),
    )
    ...
    parser.add_argument(
        "--progress",
        action="store_true",
        help="Print periodic progress updates while multiprocessing.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Print progress every N completed candidate tasks when --progress is used.",
    )
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Print each selected candidate's force constants while processing.",
    )
    ...
    if args.every_n_generations < 1:
        raise ValueError("--every-n-generations must be >= 1")
    ...
    if args.show and args.nproc > 1:
        print("WARNING: --show is ignored when --nproc > 1.")
    ...
    print(f"\nDone. Wrote outputs for {total_written} selected candidates.")
